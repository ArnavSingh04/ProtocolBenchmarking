...
import { countSuccessful } from "../../imports/shared/metrics";
...
export async function executeTests(testRunId) {
  ...
  // Route this run's engine/tester logs to its own logger (concurrency-safe).
  // The engine logs through the injected callback; the live protocol testers
  // (which still use the Meteor shim) route through the per-run registry.
  const logEntry = (payload) => addLog(testRunId, payload);
  registerRunLogger(testRunId, logEntry);

  const engine = new TestEngine(testRun.configuration, { onLog: logEntry });
  ...
  try {
    ...
    console.error(`[executeTests] run ${testRunId} failed:`, error);
    await updateRun(testRunId, {
      $set: {
        status: "failed",
        error: error.message,
    ...
    unregisterRunLogger(testRunId);
  }
}

// A run left "running" for longer than this was almost certainly interrupted
// (server restart / process exit mid-run). Longest scenario is ~60s, so 5
// minutes is a safe ceiling before we treat it as failed rather than stuck.
const STALE_RUN_MS = 5 * 60 * 1000;

export async function getRunById(testRunId) {
  const { testRuns } = await getCollections();
  const run = await testRuns.findOne({ _id: testRunId });
  if (
    run &&
    run.status === "running" &&
    run.startTime &&
    Date.now() - new Date(run.startTime).getTime() > STALE_RUN_MS
  ) {
    await testRuns.updateOne(
      { _id: testRunId },
      {
        $set: {
          status: "failed",
          error: "Run was interrupted before it could finish.",
          endTime: new Date()
        }
      }
    );
